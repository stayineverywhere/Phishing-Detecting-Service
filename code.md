...
- `ml/scoring.py`
  ...
  - **위험도 판정 로직 최적화 (v2.0)**:
    - **가중치 조정**: `z = -5 + 5*K + 5*P + 4*(AI**2) + 8*U + 8*C`
    - **AI 점수 제곱($AI^2$) 적용**: AI 모델의 확신도가 높을 때만 점수를 증폭하여 정확도 향상.
    - **긴급성(U) & 맥락(C) 강화**: 논리적인 피싱 징후(금전 요구, 압박)의 비중을 높여 오탐지 감소.
    - **키워드 필터링**: 일상어("엄마", "아빠", "확인")를 위험 키워드에서 제외.
    - **패턴 점수 보정**: `P = max(피싱 유사도 - 정상 유사도, 0)`로 계산해 정상 문장 오탐을 완화.
